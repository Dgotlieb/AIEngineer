...
|---|---|---|
| 00 | [`00-aws-setup.md`](code/00-aws-setup.md) | One-time setup: Bedrock API key, configure `.env`, run the smoke test. No Python. |
| 01 | `01-http-raw.py` | The minimum: an LLM API is just an HTTP POST. Raw `requests` to Bedrock Converse with a Bearer token. |
| 02 | `02-basic-call.py` | The same Converse call via `boto3`. Diff against 01 to see what an SDK gives you (URL, auth, retries). |
...
- Python 3.11+
- An AWS account with Bedrock model access and a Bedrock API key (`00-aws-setup.md` walks you through getting this)
