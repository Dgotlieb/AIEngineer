...
- **"In `01-http-raw.py`, list every piece of the HTTP request (URL, headers, JSON body). Which parts are 'transport' and which are 'the LLM call'? What would change if you switched models but stayed on Bedrock Converse?"**
  Forces a close look at the raw request shape before the SDK hides it in exercise 02.
