...
auth header, retries on transient errors, and a response dict instead of raw
JSON. The Converse request shape is the same; only the delivery changes. To
make that concrete, this prints the whole response envelope first, then pulls
out the one field we actually want.
