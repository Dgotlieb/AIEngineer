...
if [ ! -f "${_REPO_ROOT}/.env" ]; then
  ...
  if [ -n "${_SEED}" ]; then
    ...
    echo "    2. Fill in AWS_BEARER_TOKEN_BEDROCK, AWS_REGION,"
...
# Fail fast on the common misconfig: leftover placeholder access keys with no
# Bedrock API key. boto3 will try SigV4 with those placeholders and return a
# confusing UnrecognizedClientException instead of a clear "token missing".
akid = os.getenv("AWS_ACCESS_KEY_ID", "").strip()
placeholder_keys = akid.startswith("your-") or akid in ("", "changeme")
if not bearer or bearer.startswith("your-"):
    print("✗ AWS_BEARER_TOKEN_BEDROCK is missing or still a placeholder.", file=sys.stderr)
    print("  ACTION REQUIRED: open your .env and set AWS_BEARER_TOKEN_BEDROCK to a", file=sys.stderr)
    print("  Bedrock API key from: Bedrock console → API keys.", file=sys.stderr)
    print("  Docs: https://docs.aws.amazon.com/bedrock/latest/userguide/api-keys.html", file=sys.stderr)
    if akid and not placeholder_keys:
        print("  (Tip: remove AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY if you", file=sys.stderr)
        print("   switched to a bearer token — they are not needed.)", file=sys.stderr)
    sys.exit(1)
...
try:
    ...
    if code == "AccessDeniedException":
        if "api key" in msg.lower():
            print("  ACTION REQUIRED: Bedrock rejected the API key itself. Typical causes —", file=sys.stderr)
            print("    (a) AWS_REGION doesn't match the region where you generated the key", file=sys.stderr)
            print("        (short-term keys are region-scoped — check the console region);", file=sys.stderr)
            print("    (b) the key expired or was deactivated — generate a new one under", file=sys.stderr)
            print("        Bedrock console → API keys (prefer a long-term key for the course);", file=sys.stderr)
            print("    (c) the value in AWS_BEARER_TOKEN_BEDROCK was truncated when pasted.", file=sys.stderr)
        else:
            print("  ACTION REQUIRED: two common causes —", file=sys.stderr)
            print("    (a) the API key's IAM principal lacks bedrock:InvokeModel /", file=sys.stderr)
            print("        bedrock:CallWithBearerToken — check its attached policy;", file=sys.stderr)
        ...
        print("  ACTION REQUIRED: Bedrock rejected the credentials. Verify", file=sys.stderr)
        print("  AWS_BEARER_TOKEN_BEDROCK in .env is a valid Bedrock API key", file=sys.stderr)
        print("  (not an IAM access key), and remove any leftover", file=sys.stderr)
        print("  AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY placeholders.", file=sys.stderr)
    ...
    print("  ACTION REQUIRED: re-check AWS_BEARER_TOKEN_BEDROCK /", file=sys.stderr)
